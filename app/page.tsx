...
type View = "Overview" | "Pipeline" | "Data quality" | "Evidence search" | "Methodology";
...
export default function Workbench(){
  ...
  const guideViews:View[]=["Pipeline","Data quality","Evidence search","Methodology"];
  ...
  return <main className="case-site">
    ...
    <nav className="case-nav" aria-label="Case study navigation"><div>{(["Overview","Pipeline","Data quality","Evidence search","Methodology"] as View[]).map(item=><button key={item} className={view===item?"active":""} onClick={()=>go(item)}>{item}</button>)}</div><span>Verified bounded run · 5 Aug 2026</span></nav>

    {guidedMode&&guideIndex>=0&&<aside className="guide-dock" aria-live="polite"><div className="guide-progress"><span style={{width:`${((guideIndex+1)/guideViews.length)*100}%`}}/></div><button className="guide-close" aria-label="Exit guided walkthrough" onClick={()=>setGuidedMode(false)}>×</button><p>Guided walkthrough · {guideIndex+1} of {guideViews.length}</p><h2>{["Follow one record from source to evidence.","See how uncertainty is handled.","Test retrieval, then watch it refuse to guess.","Finish with the production path."][guideIndex]}</h2><button className="guide-next" onClick={nextGuide}>{guideIndex===guideViews.length-1?"Finish walkthrough":"Continue"}<span>→</span></button></aside>}
